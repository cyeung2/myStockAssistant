...
public class AddStock
    ...
    AddStock(MainActivity mainActivity) {
    ...
    @Override
    public void run() {
        String urlNew = DATA_URL2 + mainActivity.getAddSymbol() + DATA_URL3;
        ...
        handleStock(sbb.toString(), 0);
    }

    private void handleStock(String s, int pos) {
        if (s == null) {
        ...
        mainActivity.runOnUiThread(() -> {
            if (stocker != null) {
                mainActivity.updateStock(stocker, pos);
            }
        });
    }
